function [newEdgeImage lines linesImage] = StraightLineDetection( stat, labelledImage, props, numComps, grayImage, bwImage, edgeImage )
...
disp([ '[' stat ']' 'Iniciando filtragem dos componentes conexos']);
...
disp(['[' stat ']' 'Construindo imagem com os componentes filtrados']);
for i = 1:length(badComps)
    comp = badComps(i);
    newEdgeImage(newEdgeImage == comp) = 0;
end

disp(['[' stat ']' 'Iniciando detecção de linhas pela transformada de hough']);
[lines linesImage] = DetectLinesHough(newEdgeImage);
